...
# # Fit & Evaluate RF for Nutri-Score
...
base_rf_params = config["rf_nutri"]["base"]
best_params_by_phase = {
    "phase1_intrinsic": config["rf_nutri"]["phase1"],
    "phase2_intr+extr": config["rf_nutri"]["phase2"],
    "phase3_all_labels": config["rf_nutri"]["phase3"]
}
...
for phase_name, label_col in phases.items():
    ...
    # Train/Test-PhaseCoverage metrics
...
# 9. Save outputs
metrics_df.to_csv("results/rf_nutri_eval_metrics_train_test_core.csv", index=False)
coverage_df.to_csv("results/rf_nutri_phase_coverage_stats.csv", sep=";", index=False)
train.to_csv("results/data_def_train_with_rf_nutri_preds.csv", sep=";", index=False)
test.to_csv("results/data_def_test_with_rf_nutri_preds.csv", sep=";", index=False)

print("\nFiles written to ./results/:")
print(" - rf_nutri_eval_metrics_train_test_core.csv")
print(" - rf_nutri_phase_coverage_stats.csv")
print(" - data_def_train_with_rf_nutri_preds.csv")
print(" - data_def_test_with_rf_nutri_preds.csv")
